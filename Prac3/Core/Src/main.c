 ...
 * @authors        : Abdul-Mateen Kader, Chris Scheepers
 ******************************************************************************
 * @attention
 *
...
uint8_t eepromData[6] = {0b10101010, 0b01010101, 0b11001100, 0b00110011, 0b11110000, 0b00001111}; // initialise 8-bit array for EEPROM
uint32_t adc_val = 0;
uint32_t toggleInterval = 500; // Initial toggle interval for LED7 (2Hz)
uint16_t currentAddress = 0;   // EEPROM Address counter
/* USER CODE END PV */
...
//Added function to update Timer Period
void UpdateTimer16Period(uint32_t period);
...
void UpdateTimer16Period(uint32_t period)
{
    __HAL_TIM_SET_AUTORELOAD(&htim6, period - 1); // Set the timer period
    HAL_TIM_Base_Start(&htim6); // Restart the timer
}
